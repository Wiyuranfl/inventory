...
<head>
    ...
    <title><?php echo isset($judul) ? htmlspecialchars($judul) : "Data Supplier"; ?></title>
...
<body>
    <div id="wrapper" class="admin-layout">
            ...
            <div class="container-fluid">
                <div class="row page-header-container">
                    <div class="col-lg-8">
                        ...
                        <p class="page-subtitle">Manajemen data rekanan dan pemasok barang.</p>
                    </div>
                    <div class="col-lg-4 text-right action-buttons-container">
                        <button type="button" class="btn btn-primary btn-icon-split" data-toggle="modal" data-target="#addSupplierModal">
                        ...
                        <div class="card shadow-sm data-table-card">
                            ...
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table class="table table-hover interactive-table" id="supplierDataTable" width="100%" cellspacing="0">
                                        <thead>
                                            <tr>
                                                <th>No</th>
                                                <th>ID Supplier</th>
                                        ...
                                        <tbody>
                                            <?php
                                            if (isset($koneksi)) {
                                                // Assuming 'paging.php' is adapted for supplier data
                                                include 'paging.php';
                                            } else {
                                ...
                                <nav aria-label="Page navigation" class="mt-4">
                                    <ul class="pagination justify-content-center">
                                        <li class="page-item <?php if($halaman <= 1){ echo 'disabled'; } ?>">
                                            <a class="page-link" href="<?php if($halaman > 1){ echo "?m=supplier&s=awal&halaman=$previous"; } else { echo '#';} ?>">Previous</a>
                                        </li>
                                        <?php for($x=1; $x<=$total_halaman; $x++): ?>
                                        <li class="page-item <?php if($halaman == $x) {echo 'active';} ?>">
                                            <a class="page-link" href="?m=supplier&s=awal&halaman=<?php echo $x ?>"><?php echo $x; ?></a>
                                        </li>
                                        <?php endfor; ?>
                                        <li class="page-item <?php if($halaman >= $total_halaman) {echo 'disabled';} ?>">
                                            <a class="page-link" href="<?php if($halaman < $total_halaman) { echo "?m=supplier&s=awal&halaman=$next"; } else {echo '#';} ?>">Next</a>
                                        </li>
                                    </ul>
                                </nav>
                                <?php endif; ?>
        ...
        </div></div><div class="modal fade" id="addSupplierModal" tabindex="-1" role="dialog" aria-labelledby="addSupplierModalLabel" aria-hidden="true">
      <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content interactive-modal">
          <div class="modal-header">
            <h5 class="modal-title" id="addSupplierModalLabel">Tambah Data Supplier Baru</h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
              <span aria-hidden="true">×</span>
            </button>
          </div>
          <form action="?m=supplier&s=simpan" method="POST" class="needs-validation" novalidate>
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            <label for="nama_sup">Nama Supplier <span class="text-danger">*</span></label>
                            <input type="text" class="form-control" id="nama_sup" name="nama_sup" placeholder="Masukkan nama perusahaan atau perorangan" required>
                            <div class="invalid-feedback">Nama supplier wajib diisi.</div>
                        </div>
                        <div class="form-group">
                            <label for="kontak_sup">Nama Kontak (Person)</label>
                            <input type="text" class="form-control" id="kontak_sup" name="kontak_sup" placeholder="Masukkan nama PIC">
                        </div>
                    </div>
                    <div class="col-md-6">
                         <div class="form-group">
                            <label for="telepon_sup">Nomor Telepon <span class="text-danger">*</span></label>
                            <input type="tel" class="form-control" id="telepon_sup" name="telepon_sup" placeholder="Contoh: 08123456789" required>
                            <div class="invalid-feedback">Nomor telepon wajib diisi.</div>
                        </div>
                    </div>
                </div>
                <div class="form-group">
                    <label for="alamat_sup">Alamat Lengkap</label>
                    <textarea class="form-control" id="alamat_sup" name="alamat_sup" rows="3" placeholder="Masukkan alamat lengkap supplier"></textarea>
                </div>
            </div>
